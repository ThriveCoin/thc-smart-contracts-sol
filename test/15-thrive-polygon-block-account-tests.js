  ...
  contract('block account tests', (accounts) => {
    ...
    before(async () => {
      ...
      await contract.deposit(
        accounts[0],
        web3.eth.abi.encodeParameter('uint256', '999700000'),
        { from: accounts[0] }
      )
    ...
    it('deposit should fail when receiver is blocked', async () => {
      try {
        await contract.deposit(accounts[1], web3.eth.abi.encodeParameter('uint256', 500), { from: accounts[0] })
        throw new Error('Should not reach here')
      } catch (err) {
        assert.strictEqual(
          err.message.includes('ERC20Blockable: receiver account should be not be blocked'),
    ...
    it('deposit should fail when caller is blocked', async () => {
      await contract.updateChildChainManager(accounts[1], { from: accounts[0] })

      try {
        await contract.deposit(accounts[0], web3.eth.abi.encodeParameter('uint256', 500), { from: accounts[1] })
        throw new Error('Should not reach here')
      } catch (err) {
        assert.strictEqual(
          err.message.includes('ERC20Blockable: caller account should be not be blocked'),
    ...
    it('withdraw should fail when caller is blocked', async () => {
      try {
        await contract.withdraw(25, { from: accounts[1] })
        throw new Error('Should not reach here')
      } catch (err) {
        assert.strictEqual(
          err.message.includes('ERC20Blockable: sender account should be not be blocked'),
