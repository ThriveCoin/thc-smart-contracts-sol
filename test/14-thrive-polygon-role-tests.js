...
describe('ThriveCoinERC20TokenPolygon', () => {
  contract('role tests', (accounts) => {
    ...
    it('hasRole should return false when user does not have role', async () => {
      const res = await contract.hasRole(PAUSER_ROLE, accounts[3])
    ...
    it('owner should have all roles by default', async () => {
      const owner = await contract.owner.call()
    ...
    it('only admin role can grant roles', async () => {
      ...
      const hasRole = await contract.hasRole(PAUSER_ROLE, accounts[3])
    ...
    it('grantRole should emit RoleGranted event', async () => {
      const res = await contract.grantRole(PAUSER_ROLE, accounts[4], { from: accounts[0] })
    ...
    it('only admin role can revoke role', async () => {
      await contract.revokeRole(PAUSER_ROLE, accounts[3], { from: accounts[0] })
      const hasRole = await contract.hasRole(PAUSER_ROLE, accounts[3])
    ...
    it('revokeRole should emit RoleRevoked event', async () => {
      const res = await contract.revokeRole(PAUSER_ROLE, accounts[4], { from: accounts[0] })
      const txLog = res.logs[0]
    ...
    it('role members be enumerable', async () => {
      const pausers = [accounts[0], accounts[2], accounts[3]]
      const length = await contract.getRoleMemberCount.call(PAUSER_ROLE)

      for (let index = 0; index < length; index++) {
        const pauser = await contract.getRoleMember(PAUSER_ROLE, index)
        assert.strictEqual(pauser, pausers[index])
      }
    })
    ...
    it('withdraw can be done by any user', async () => {
    ...
    it('pause can be done only by PAUSER_ROLE', async () => {
      ...
      try {
        await contract.pause({ from: accounts[4] })
        throw new Error('Should not reach here')
      } catch (err) {
        assert.strictEqual(
          err.message.includes('ThriveCoinERC20TokenPolygon: must have pauser role to pause'),
          true
        )
      }
    })
    ...
    it('unpause can be done only by PAUSER_ROLE', async () => {
      ...
      try {
        await contract.unpause({ from: accounts[4] })
        throw new Error('Should not reach here')
      } catch (err) {
        assert.strictEqual(
          err.message.includes('ThriveCoinERC20TokenPolygon: must have pauser role to unpause'),
          true
        )
      }
    })
    ...
    it('blockAccount can be done only by DEFAULT_ADMIN_ROLE', async () => {
      ...
      try {
        ...
        assert.strictEqual(
          err.message.includes('ThriveCoinERC20TokenPolygon: caller must have admin role to block the account'),
          true
        )
      }
    })
    ...
    it('unblockAccount can be done only by DEFAULT_ADMIN_ROLE', async () => {
      ...
      try {
        ...
        assert.strictEqual(
          err.message.includes('ThriveCoinERC20TokenPolygon: caller must have admin role to unblock the account'),
          true
        )
      }
    })
